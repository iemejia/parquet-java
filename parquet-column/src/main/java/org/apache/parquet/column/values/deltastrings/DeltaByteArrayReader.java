...
public class DeltaByteArrayReader extends ValuesReader implements RequiresPreviousReader {
  ...
  public void readBinaries(Binary[] dest, int offset, int count) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (prefixLength != 0) {
        byte[] out = new byte[length];
        // Use toByteBuffer().get() instead of getBytesUnsafe() + System.arraycopy:
        // avoids a temporary byte[] allocation when the Binary is ByteBuffer-backed
        // (which suffixes from DeltaLengthByteArrayValuesReader always are).
        previous.toByteBuffer().get(out, 0, prefixLength);
        suffix.toByteBuffer().get(out, prefixLength, suffix.length());
        previous = Binary.fromConstantByteArray(out);
      } else {
        previous = suffix;
      }
