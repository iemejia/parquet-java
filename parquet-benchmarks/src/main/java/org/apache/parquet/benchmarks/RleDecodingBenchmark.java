 ...
 *
 * <p>PLAIN (V1) boolean decoding benchmarks live in
 * {@link PlainDecodingBenchmark}. Synthetic dictionary-index RLE benchmarks
 * live in {@link RleDictionaryIndexDecodingBenchmark}.
 *
...
public class RleDecodingBenchmark {
  ...
  private byte[] rlePage;

  // Pre-allocated batch destination array (avoid per-invocation allocation artifact)
  ...
  public void setup() throws IOException {
    ...
    rlePage = w.getBytes().toByteArray();
    w.close();
  ...
  public void decodeBoolean(Blackhole bh) throws IOException {
    ...
    r.initFromPage(VALUE_COUNT, ByteBufferInputStream.wrap(ByteBuffer.wrap(rlePage)));
