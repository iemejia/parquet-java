...
read         | Reading files with different compression and writer versions.
write        | Writing files with different compression and writer versions.
filewrite    | CPU-only file writing (BlackHole sink, no filesystem I/O).
fileread     | File reading from temp files with different codecs/versions.
encoding     | INT32 and BINARY encoding/decoding micro-benchmarks.
concurrent   | Multi-threaded concurrent read/write benchmarks.
...
EOF
  ...
  ( cd "$REPO_ROOT" && ./mvnw --projects parquet-benchmarks -am -DskipTests -Denforcer.skip=true clean package )
  ...
  "write")
    ...
    ;;
  "filewrite")
    BENCHMARK_REGEX="org.apache.parquet.benchmarks.FileWriteBenchmark"
    ;;
  "fileread")
    BENCHMARK_REGEX="org.apache.parquet.benchmarks.FileReadBenchmark"
    ;;
  "encoding")
    BENCHMARK_REGEX="org.apache.parquet.benchmarks.(Int|Binary)EncodingBenchmark"
    ;;
  "concurrent")
    BENCHMARK_REGEX="org.apache.parquet.benchmarks.ConcurrentReadWriteBenchmark"
    ;;
  "checksum")
